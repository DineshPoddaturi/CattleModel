...
prices_predict_co4_merge111 <- prices_predict_co4_merge
...
percentChange_slRev <- rev_sl %>% mutate(percentChange_slRev_model = ((slRev_post - slRev_model)/slRev_model)*100,
                                         ...
                                         ) %>% select(Year, percentChange_slRev_model, percentChange_slRev_obs) %>% filter(Year>2009)
...
percentChange_clRev <- rev_cl %>% mutate(percentChange_clRev_model = ((clRev_post - clRev_model)/clRev_model)*100,
                                         ...
                                         ) %>% select(Year, percentChange_clRev_model, percentChange_clRev_obs) %>% filter(Year>2009)
...
percentChange_tRev <- rev_total %>% mutate(percentChange_tRev_model = ((totalRev_post - totalRev_model)/totalRev_model)*100,
                                           ...
                                           ) %>% select(Year, percentChange_tRev_model, percentChange_tRev_obs) %>% filter(Year>2009)
...
revDiff_costs_sl <- revDiff_costs_sl %>% mutate(diffRevCost_sl_obs = slRev_diff_obs - costSupply_sl_obs, 
                                                diffRevCost_sl_model = slRev_diff_model - costSupply_sl_model)
revDiff_costs_cl <- revDiff_costs_cl %>% mutate(diffRevCost_cl_obs = clRev_diff_obs - costSupply_cl_obs,
                                                diffRevCost_cl_model = clRev_diff_model - costSupply_cl_model)
revDiff_costs_t <- revDiff_costs_t %>% mutate(diffRevCost_t_obs = totalRev_diff_obs - costSupply_t_obs,
                                              diffRevCost_t_model = totalRev_diff_model - costSupply_t_model)
...
### Let me change the sl and cl as well and see how things change
# supp_sl_1 <- supp_sl %>% filter(Year<=2008) %>% mutate(sl = Bill_meatLb_sl) %>% select(Year, sl)
# sl_1 <- demand_predict_est %>% mutate(Year = Year - 1) %>% select(Year, sl) %>% filter(Year>2008)
# supp_sl_1 <- rbind(supp_sl_1, sl_1)
# 
# supp_cl_1 <- supp_cl %>% filter(Year<=2008) %>% mutate(cl = Bill_meatLb_cl) %>% select(Year, cl)
# cl_1 <- demand_predict_est %>% mutate(Year = Year - 1) %>% select(Year, cl) %>% filter(Year>2008)
# supp_cl_1 <- rbind(supp_cl_1, cl_1)
# 
# totalDisappearedNew_1 <- totalDisappearedNew %>% filter(Year<=2008) %>% mutate(Demand = total_meat_bill) %>% select(Year, Demand)
# disappear_1 <- demand_predict_est %>% mutate(Year = Year - 1) %>% select(Year, Demand) %>% filter(Year>2008)
...
prices_predict_co4_merge_1 <- merge(prices_predict_co4_1, merge(prices_predict_est,prices_costs_obs)) %>% 
  mutate(ps_hat = ps_hat * 100, pc_hat = pc_hat * 100, hc_hat = hc_hat * 100, 
         ps_est = ps_est * 100, pc_est = pc_est * 100, hc_est = hc_est * 100,
         ps = ps * 100, pc = pc * 100, hc = hc * 100,) %>% select(Year, ps, ps_est, ps_hat, pc, pc_est, pc_hat, hc, hc_est, hc_hat)
demand_predict_co4_merge_1 <- merge(demand_predict_co4_1, merge(demand_predict_est,demand_obs)) %>%
  select(Year, Demand, Demand_est, Demand_hat, sl, sl_est, sl_hat, cl, cl_est, cl_hat)
...
slaughterPrices_plot_co4_1 <- prices_predict_co4_merge_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=ps_est,color="Model estimate"))+
  geom_point(aes(y=ps_est,color="Model estimate"))+ geom_line(aes(y=ps_hat, color="Estimate with added costs"))+
  geom_point(aes(y=ps_hat,color="Estimate with added costs")) + geom_line(aes(y=ps, color="Observed"))+
  geom_point(aes(y=ps,color="Observed")) + 
  labs(x="Year", y="Slaughter Prices (\\$/cwt)", colour = "") + theme_classic() + 
...
cullPrices_plot_co4_1 <- prices_predict_co4_merge_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=pc,color="Estimate without added costs"))+geom_point(aes(y=pc,color="Estimate without added costs")) + geom_line(aes(y=pc_hat, color="Estimate with added costs")) + geom_point(aes(y=pc_hat,color="Estimate with added costs")) + 
  labs(x="Year", y="Culled Prices (\\$/cwt)", colour="") + theme_classic() + 
  scale_x_continuous(name="Year", breaks=c(seq(prices_predict_co4_merge_1$Year[1], prices_predict_co4_merge_1$Year[nrow(prices_predict_co4_merge_1)]))) 

holdingCosts_plot_co4_1 <- prices_predict_co4_merge_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=hc,color="Estimate without added costs"))+geom_point(aes(y=hc,color="Estimate without added costs")) +geom_line(aes(y=hc_hat, color="Estimate with added costs")) + geom_point(aes(y=hc_hat,color="Estimate with added costs")) + 
  labs(x="Year", y="Holding Costs (\\$/cwt)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(prices_predict_co4_merge_1$Year[1], prices_predict_co4_merge_1$Year[nrow(prices_predict_co4_merge_1)])))


stock_slaughter_co4_1 <- demand_predict_co4_merge_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=sl,color="Estimate without added costs"))+geom_point(aes(y=sl,color="Estimate without added costs")) +geom_line(aes(y=sl_est, color="Estimate with added costs")) + geom_point(aes(y=sl_est,color="Estimate with added costs")) + 
  labs(x="Year", y="Slaughter meat (in Billion pounds)", colour="") + theme_classic()+ 
...
stock_cull_co4_1 <- demand_predict_co4_merge_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=cl,color="Estimate without added costs"))+geom_point(aes(y=cl,color="Estimate without added costs")) +geom_line(aes(y=cl_est, color="Estimate with added costs")) + geom_point(aes(y=cl_est,color="Estimate with added costs")) + 
  labs(x="Year", y="Culled meat (in Billion pounds)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(demand_predict_co4_merge_1$Year[1],demand_predict_co4_merge_1$Year[nrow(demand_predict_co4_merge_1)])))
...
percentChange_price_1 <- prices_predict_co4_merge_1 %>% mutate(percentChange_ps_model = ((ps_hat - ps_est)/ps_est)*100,
                                                               percentChange_ps_obs = ((ps_hat - ps)/ps)*100,
                                                               percentChange_pc_model = ((pc_hat - pc_est)/pc_est)*100,
                                                               percentChange_pc_obs = ((pc_hat - pc)/pc)*100,
                                                               percentChange_hc_model = ((hc_hat - hc_est)/hc_est)*100,
                                                               percentChange_hc_obs = ((hc_hat - hc)/hc)*100,) %>% select(
...
percentChange_demand_1 <- demand_predict_co4_merge_1 %>% mutate(percentChange_demand_model = ((Demand_hat - Demand_est)/Demand_est)*100,
                                                                percentChange_demand_obs = ((Demand_hat - Demand)/Demand)*100,
                                                                percentChange_sl_model = ((sl_hat - sl_est)/sl_est)*100,
                                                                percentChange_sl_obs = ((sl_hat - sl)/sl)*100,
                                                                percentChange_cl_model = ((cl_hat - cl_est)/cl_est)*100,
                                                                percentChange_cl_obs = ((cl_hat - cl)/cl)*100,) %>% select(
...
percentChange_slRev_1 <- rev_sl_1 %>% mutate(percentChange_slRev = ((slRev_post - slRev_pre)/slRev_pre)*100) %>% select(Year, percentChange_slRev) %>% filter(Year>2009)

percentChange_clRev_1 <- rev_cl_1 %>% mutate(percentChange_clRev = ((clRev_post - clRev_pre)/clRev_pre)*100) %>% select(Year, percentChange_clRev) %>% filter(Year>2009)

percentChange_tRev_1 <- rev_total_1 %>% mutate(percentChange_tRev = ((totalRev_post - totalRev_pre)/totalRev_pre)*100) %>% select(Year, percentChange_tRev) %>% filter(Year>2009)
...
revDiff_costs_sl_1 <- revDiff_costs_sl_1 %>% mutate(diffRevCost_sl_model = slRev_diff_model - costSupply_sl_model,
                                                    diffRevCost_sl_obs = slRev_diff_obs - costSupply_sl_obs)
revDiff_costs_cl_1 <- revDiff_costs_cl_1 %>% mutate(diffRevCost_cl_model = clRev_diff_model - costSupply_cl_model,
                                                    diffRevCost_cl_obs = clRev_diff_obs - costSupply_cl_obs)
revDiff_costs_t_1 <- revDiff_costs_t_1 %>% mutate(diffRevCost_t_model = totalRev_diff_model - costSupply_t_model,
                                                  diffRevCost_t_obs = totalRev_diff_obs - costSupply_t_obs)
...
slaughterPrices_plot_co41_1 <- prices_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=ps,color="Observed"))+geom_point(aes(y=ps,color="Observed"))+geom_line(aes(y=ps_hat, color="Estimate with added costs"))+geom_point(aes(y=ps_hat,color="Estimate with added costs")) + 
  labs(x="Year", y="Slaughter Prices (\\$/cwt)", colour = "") + theme_classic() + 
  scale_x_continuous(name="Year", breaks=c(seq(prices_predict_co4_merge1_1$Year[1], prices_predict_co4_merge1_1$Year[nrow(prices_predict_co4_merge1_1)]))) 

cullPrices_plot_co41_1 <- prices_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=pc,color="Observed"))+geom_point(aes(y=pc,color="Observed")) + geom_line(aes(y=pc_hat, color="Estimate with added costs")) + geom_point(aes(y=pc_hat,color="Estimate with added costs")) + 
  labs(x="Year", y="Culled Prices (\\$/cwt)", colour="") + theme_classic() + 
  scale_x_continuous(name="Year", breaks=c(seq(prices_predict_co4_merge1_1$Year[1], prices_predict_co4_merge1_1$Year[nrow(prices_predict_co4_merge1_1)]))) 

holdingCosts_plot_co41_1 <- prices_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=hc,color="Observed"))+geom_point(aes(y=hc,color="Observed")) +geom_line(aes(y=hc_hat, color="Estimate with added costs")) + geom_point(aes(y=hc_hat,color="Estimate with added costs")) + 
  labs(x="Year", y="Holding Costs (\\$/cwt)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(prices_predict_co4_merge1_1$Year[1], prices_predict_co4_merge1_1$Year[nrow(prices_predict_co4_merge1_1)])))


stock_slaughter_co41_1 <- demand_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=sl,color="Observed"))+geom_point(aes(y=sl,color="Observed")) +geom_line(aes(y=sl_est, color="Estimate with added costs")) + geom_point(aes(y=sl_est,color="Estimate with added costs")) + 
  labs(x="Year", y="Slaughter meat (in Billion pounds)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(demand_predict_co4_merge1_1$Year[1],demand_predict_co4_merge1_1$Year[nrow(demand_predict_co4_merge1_1)])))

stock_cull_co41_1 <- demand_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=cl,color="Observed"))+geom_point(aes(y=cl,color="Observed")) +geom_line(aes(y=cl_est, color="Estimate with added costs")) + geom_point(aes(y=cl_est,color="Estimate with added costs")) + 
  labs(x="Year", y="Culled meat (in Billion pounds)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(demand_predict_co4_merge1_1$Year[1],demand_predict_co4_merge1_1$Year[nrow(demand_predict_co4_merge1_1)])))

demand_co41_1 <- demand_predict_co4_merge1_1 %>% ggplot(aes(x=Year))+geom_line(aes(y=Demand,color="Observed"))+geom_point(aes(y=Demand,color="Observed")) +geom_line(aes(y=demand_est, color="Estimate with added costs")) + geom_point(aes(y=demand_est,color="Estimate with added costs")) + 
  labs(x="Year", y="Demand meat (in Billion pounds)", colour="") + theme_classic()+ 
  scale_x_continuous(name="Year", breaks=c(seq(demand_predict_co4_merge1_1$Year[1],demand_predict_co4_merge1_1$Year[nrow(demand_predict_co4_merge1_1)])))
