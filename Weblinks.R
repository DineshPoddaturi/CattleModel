...
# https://apps.fas.usda.gov/psdonline/app/index.html#/app/advQuery     (for beef demand and cattle totals)


# https://quickstats.nass.usda.gov/#B5BAD398-03E0-38D6-B1EA-D120F5AB94E8 (for prices received for fed cattle and cull cattle)
...
############################################################################
############### New numbers for K, imports and exports #####################
############################################################################

# tot <- read_excel("Data/CattleTotals.xlsx")%>% as.data.frame()
# 
# tots <- data.frame(t(tot[-1]))
# colnames(tots) <- tot[,1]
# 
# tots <- tots %>% mutate(Year = c(rownames(tots)))
# tots <- tots %>% select(Year,everything())
# tots[,-1] <- tots[,-1] * 1000
# 
# totalHerd <- tots %>% select(Year,`Beef Cows Beg. Stocks`,Imports,Exports)
...
exports_imports <- meat_billion %>% select(Year,Imports,Exports)


################################# exports and imports ###########################

# exports <- totalHerd %>% filter(Year>=2008 & Year<=2018) %>% select(Year,Exports) %>% as.data.frame()
# imports <- totalHerd %>% filter(Year>=2008 & Year<=2018) %>% select(Year,Imports) %>% as.data.frame()
...
# slaughter <- slaughter %>% filter(Year>=2001 & Year<2018) %>% na.omit()
# 
...
# dairy_Supp <- dairy_Supp %>% mutate(Year = seq(2009,2017), Bill_meatLb = (dairyBeef * avg_weight * phi)/1000000000) %>% select(Year, everything())
...
#   labs(x="Year", y="Holding Costs (\\$/cwt)", colour="") + theme_classic()+ scale_x_continuous(name="Year", breaks=c(seq(2009,2017))) 
# 
# combinedPlot <- ggarrange(slaughter_plot, cull_plot, holding_plot, ncol=1, common.legend = TRUE, legend = "right")
# print(combinedPlot)
# 
# dev.off()
...
Master_sl_cl_head[,-1] <- round(Master_sl_cl_head[,-1],2)

stock_slaughter_head <- Master_sl_cl_head %>% ggplot(aes(x=Year))+geom_line(aes(y=sl,color="Observed"))+geom_point(aes(y=sl,color="Observed")) +geom_line(aes(y=sl_hat, color="Estimate")) + geom_point(aes(y=sl_hat,color="Estimate")) + 
  labs(x="Year", y="Slaughtered animals (in million)", colour="Stock") + theme_classic()+ scale_x_continuous(name="Year", breaks=c(seq(2009,2017)))

stock_cull_head <- Master_sl_cl_head %>% ggplot(aes(x=Year))+geom_line(aes(y=cl,color="Observed"))+geom_point(aes(y=cl,color="Observed")) +geom_line(aes(y=cl_hat, color="Estimate")) + geom_point(aes(y=cl_hat,color="Estimate")) +
  labs(x="Year", y="Culled animals (in million)", colour="Stock") + theme_classic()+ scale_x_continuous(name="Year", breaks=c(seq(2009,2017)))
