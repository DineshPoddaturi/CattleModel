...
estProj_PCIIV <- merge(EQestObsPC_Medians %>% mutate(pcMedian = pcMedian*100),proj_Q_PIIVII %>% select(Year, Pc), all=TRUE)


EQestObsSL_Medians <- EQestObsSLNIII %>% select(Year, slMedian, slSM) %>% filter(Year > 2015)

estProj_SLIIV <- merge(EQestObsSL_Medians ,proj_Q_PIIVII %>% select(Year, Sl), all=TRUE)


EQestObsCL_Medians <- EQestObsCLNIII %>% select(Year, clMedian, clSM) %>% filter(Year > 2015)

estProj_CLIIV <- merge(EQestObsCL_Medians ,proj_Q_PIIVII %>% select(Year, Cl), all=TRUE)



estProj_PS_plots <- estProj_PSIIV %>% filter(Year >=2018 & Year <= 2031) %>% ggplot(aes(x=Year)) +
  ...
  scale_y_continuous(name="Fed Cattle Price", limits = c(110,145,by=5)) +  theme_classic() + 
...
estProj_PC_plots <- estProj_PCIIV %>% filter(Year >=2018 & Year <= 2031) %>% ggplot(aes(x=Year)) +
  ...
  scale_y_continuous(name="Cull Cow Price", limits = c(65,80,by=5)) +  theme_classic() + 
  theme(legend.position="bottom", legend.box = "horizontal",text = element_text(size = 12)) +
  theme(legend.title=element_blank()) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ 
  theme(legend.text = element_text(margin = margin(r = 30, unit = "pt")))


estProj_SL_plots <- estProj_SLIIV %>% filter(Year >=2018 & Year <= 2031) %>% ggplot(aes(x=Year)) +
  ...
  scale_y_continuous(name="Fed Cattle Production", limits = c(22,25,by=5)) +  theme_classic() + 
  theme(legend.position="bottom", legend.box = "horizontal",text = element_text(size = 12)) +
  theme(legend.title=element_blank()) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ 
  theme(legend.text = element_text(margin = margin(r = 30, unit = "pt")))


estProj_CL_plots <- estProj_CLIIV %>% filter(Year >=2018 & Year <= 2031) %>% ggplot(aes(x=Year)) +
  ...
  scale_y_continuous(name="Cull Cow Production", limits = c(2.0,4.0,by=1)) +  theme_classic() + 
  theme(legend.position="bottom", legend.box = "horizontal",text = element_text(size = 12)) +
  theme(legend.title=element_blank()) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ 
  theme(legend.text = element_text(margin = margin(r = 30, unit = "pt")))
