...
demand_predict_expN <- data.frame(Year = predict_df$Year+1, demand_est = numeric(nrow(predict_df)), sl_est = numeric(nrow(predict_df)), cl_est = numeric(nrow(predict_df)))

# demand_predict <- data.frame(Year = predict_df$Year+1, demand_est = numeric(nrow(predict_df)))

prices_predict_expN <- data.frame(Year = predict_df$Year+1, ps_hat = numeric(nrow(predict_df)), pc_hat = numeric(nrow(predict_df)), hc_hat = numeric(nrow(predict_df)))

parameters_expN <- data.frame(Year = predict_df$Year+1, mu_tilde = numeric(nrow(predict_df)), s_tilde = numeric(nrow(predict_df)))
adj_factor_expN <- data.frame(Year = predict_df$Year+1, adj = numeric(nrow(predict_df)))
shares_slcl_expN <- data.frame(Year = predict_df$Year+1, share_pre = numeric(nrow(predict_df)), 
...
for(i in 1:(nrow(predict_df)-2)){
  ...
  params_t1 <- mu_s_tildes(sl=sl_t1_hat, cl=cl_t1_hat, ps = ps_t, pc = pc_t, thetas = c(1,1))
  parameters_expN$mu_tilde[i] <- params_t1[1]
  parameters_expN$s_tilde[i] <- params_t1[2]
  
  p <- c(ps_t, pc_t, hc_t)
  # p <- c(0.5,0.3,0.2)
  sl <- sl_t1_hat
  cl <- cl_t1_hat
  A <- demand_t1_hat*(1 - 0.25)
  mu_Tilde <- params_t1[1]
  s_Tilde <- params_t1[2]
  ...
  slShare_t <- (exp((params_t1[1] - ((ps_hat_t1 - pc_hat_t1))/phi)/params_t1[2]))
  shares_slcl_expN$share_post[i] <- slShare_t
  
  prices_predict_expN$ps_hat[i] <- ps_hat_t1
  prices_predict_expN$pc_hat[i] <- pc_hat_t1
  prices_predict_expN$hc_hat[i] <- hc_hat_t1
  demand_predict_expN$demand_est[i] <- demand_t1_hat
  demand_predict_expN$sl_est[i] <- sl_t1_hat
  demand_predict_expN$cl_est[i] <- cl_t1_hat
}

pricesMerge_new_expN <- merge(prices_predict_expN,prices_costs) %>% filter(ps_hat>0) %>% select(Year, ps, ps_hat, pc, pc_hat, hc, hc_hat)

demandMerge_new_expN <- merge(demand_new, demand_predict_expN) %>% select(Year, Demand, demand_est) %>% filter(demand_est>0)
demandMerge_new_expN$Year <- as.numeric(demandMerge_new_expN$Year)
...
> shares_co4
  Year   shares shares_2009
1 2010 7.812906    6.495846
2 2011 7.479883    6.950602
3 2012 6.912212    6.631910
4 2013 7.696693    6.825991
5 2014 7.173944    6.692927
6 2015 6.827108    5.771584
7 2016 6.973252    6.042656
8 2017 0.000000    0.000000
9 2018 0.000000    0.000000
