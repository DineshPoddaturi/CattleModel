...
getPsPcEpsEpc_FMD_EQ_OPT <- function(PsM, PcM, EPsM, EPcM, HcM, SlNew, ClNew, 
  ...
  if(depops == 5){
    ...
    pcNew_expected_lo <- pcNew_expected - 0.05
...
slHistMax <- FMD_AllDF_EQ %>% filter(Year <= 2021 & Year >= 2000) %>% select(slSM) %>% max()
clHistMax <- FMD_AllDF_EQ %>% filter(Year <= 2021 & Year >= 2000) %>% select(clSM) %>% max()

slHistMin <- FMD_AllDF_EQ %>% filter(Year <= 2021 & Year >= 2000) %>% select(slSM) %>% min()
clHistMin <- FMD_AllDF_EQ %>% filter(Year <= 2021 & Year >= 2000) %>% select(clSM) %>% min()
...
simOptimisticFMD <- function(dePopR, modelParamsEQ_PreFMD, exports_percentK, nn, Stock, holdingCostsFuturesFMD){
  ...
  for(i in 1: nrow(proj_Q_P_PostFMD)){
    ...
    if(i>=3){
      ...
      ### Here I check whether the live animal exports are greater than 2021 exports
      ### If yes, then add those animals back in the stock. Note that this is a conservative approach
      while(expTot > exports_2021_Live){
        expTot <- expTot - 1000
      }
    ...
    if(i<3){
      ccY1 <- gFMD * (KTwo - clTwoNew)
    }else{
      
      if(expUpdatek3[i-2]==1 || expUpdateK[i-2]==1){
        ccY1 <- gFMD * (KTwo - clTwo) - slExprtsHead
      }else{
        ccY1 <- gFMD * (KTwo - clTwo) - slExprtsHead
      }
    }
    ...
    if(i<2){
      ...
      if(expUpdatek3[i-1]==1 || expUpdateK[i-1]==1){
    ...
    if(i>=3){
      if(k_old_headFMD < ccY1 * repRatio){
        impUpdatek3[i] <- 1
      }else if(k_old_headFMD > k3HistMax){
        expUpdatek3[i] <- 1
      }
    }
    ...
    if(dePopR == 20){
      if(i<3){
        approxKFMD <- KOneNew +
          (0.5 * (delta^2) * (ccY1)) - k_old_headFMD_OG - exprtsFMD
      }else{
        approxKFMD <- KOneNew +
          (0.5 * (delta^2) * (ccY1)) - k_old_headFMD_OG + 
      ...
      if(i<3){
        ...
        approxKFMD <- KOneNew + k_old_headFMD - clExprtsHead
      }
    ...
    if(i >= 3){
      ...
      approxKFMD_OG <- approxKFMD
      
      #### Here I will get how many animals are exported from the condition 
      #### the approximate stock is greater than the historical median
      while(approxKFMD > KHistMax){
        approxKFMD <- approxKFMD - 10000
        expUpdateK[i] <- 1
      }
      
      if(expUpdateK[i] == 1){
        
        exprtsFMDK <- approxKFMD_OG - approxKFMD
        
        mergedForecastFMD_Proj$Exports[mergedForecastFMD_Proj$Year == yearIFMD] <- 
          exprtsFMDK
        
        if(expUpdatek3[i] == 1){
          k_old_headFMD <- k_old_headFMD_OG1 - exprtsFMDK
        }
      ...
      if(impUpdatek3[i]==1){
        ...
        while(k_old_headFMD < ccY1 * repRatio){
          k_old_headFMD <- k_old_headFMD + 10000
        }
        
        imprtsFMDk3 <- (k_old_headFMD - k3_headFMD_OG)
        
        k_old_headFMD <- k3_headFMD_OG + imprtsFMDk3
        approxKFMD <- approxKFMD + imprtsFMDk3
        
        approxKFMD_OG1 <- approxKFMD
        imprtsFMDK <- imprtsFMDk3
        
        if(approxKFMD < KHistMed){
          while(imprtsFMDK < impHistMed){
            approxKFMD <- approxKFMD + 10000
            imprtsFMDK <- imprtsFMDK + (approxKFMD - approxKFMD_OG1)
            approxKFMD_OG1 <- approxKFMD
            if(approxKFMD >= KHistMed){
              break
            }
        ...
        k_old_headFMD <- k_old_headFMD + (imprtsFMDK - imprtsFMDk3)
    ...
    hcM_pre <- holdingCostsFuturesFMD5$hc[i]
    ...
    if(k9_OldFMD == 1){
      ...
      # if(mergedForecastFMD_Proj$k8[mergedForecastFMD_Proj$Year == yearIFMD-1]>0){
      #   mergedForecastFMD_Proj$k9[mergedForecastFMD_Proj$Year == yearIFMD] <-
      #     mergedForecastFMD_Proj %>% filter(Year == yearIFMD) %>%
      #     mutate(k9 = K - (k3+k4+k5+k6+k7+k8)) %>% mutate(k9 = if_else(k9 < 0, 0, k9)) %>%
      #     select(k9) %>% as.numeric()
      # }else{
      mergedForecastFMD_Proj$k9[mergedForecastFMD_Proj$Year == yearIFMD] <-
      ...
      # k9NextFMD <- mergedForecastFMD_Proj %>% filter(Year == yearIFMD-1) %>%
      #   mutate(k9 = delta * k8) %>% select(k9) %>% as.numeric()
    ...
    if(clNew1FMD > clNewFMD_OG){
      ...
      if(i==1){
        clNew <- clNew1FMD + clNew1FMD * (exports_percentK/100)
      }else if(i==2){
        clNew <- clNew1FMD + clNew1FMD * (exports_percentK/100)
      }else{
        clNew <- clNew1FMD - clNew1FMD * (exports_percentMeat/100)
      }
      clNewFMD <- clNew
    }
...
simOptimisticFMD <- function(dePopR, modelParamsEQ_PreFMD, exports_percentK, 
  ...
  for(i in 1: nrow(proj_Q_P_PostFMD)){
    ...
    if(i==1){
      ...
      slExprtsLive <- expLM$coefficients[1] * (slNewFMD * (1000000000/fedAvgFMD))
    ...
    if(i == 1){
      ...
      slExprtsMeat <- (expBeefLM$coefficients[1] * slNewFMD) %>% as.numeric()
      clExprtsMeat <- (expBeefLM$coefficients[1] * clNewFMD) %>% as.numeric()
    ...
    # KTwo <- mergedForecastFMD_Proj %>% filter(Year == yearIFMD-2) %>% 
    #   mutate(sK = k3+k4+k5+k6+k7+k8+k9+k10) %>% select(sK)
    # 
    # if(i<3){
    #   ccY1 <- gFMD * (KTwo)
    # }else{
    #   ccY1 <- gFMD * (KTwo) - slExprtsHead
    # }
    ...
    if(i>1){
      if(impUpdatek3[i-1]>0){
        ccY1 <- ccY1 + mergedForecastFMD_Proj$Imports[mergedForecastFMD_Proj$Year == yearIFMD-1] * delta
      }
    }
    ...
    if(i<3){
      ...
      k_old_headFMD <- KOneNew + ((delta^2) * 0.5 * (ccY1)) - slDemHeadFMD - slExprtsHead
    }
    ...
    if(impUpdatek3[i]==1){
      ...
      # k_old_headFMD <- k_old_headFMD + imprtsFMDK
      # approxKFMD <- approxKFMD + imprtsFMDK
    }
...
simPessimisticFMD <- function(dePopR, modelParamsEQ_PreFMD, exports_percentK, 
  ...
  for(i in 1: nrow(proj_Q_P_PostFMD)){
    ...
    if(i==1){
      ...
      # capAFMD_DollarsOG <- capAFMD * sh_pre * psM_pre + capAFMD * (1-sh_pre) * pcM_pre
      capAFMD_DollarsOG <- slSM_pre * psM_pre + clSM_pre * pcM_pre
      exports_Live <- exports_2021_Live * (1-(dePopR/100))
    }else if(i>=2 && i <=5){
      exports_Live <- exports_2021_Live
    }
    ...
    if(slNewFMD < slHistMin || clNewFMD < clHistMin){
      ...
      impBeef[i] <- (slNewFMD + clNewFMD) - (slNewFMD_OG_New + clNewFMD_OG_New)
    }
    ...
    if(i>5){
      ...
      # if(mergedForecastFMD_Proj$K[i-1]<KHistMin){
      #   
      #   slExprtsHead <- 0
      #   clExprtsHead <- 0
      #   expTotOG <- slExprtsHead + clExprtsHead
    ...
    ##### Here after three years I check whether we need imports to replenish the stocks and then import animals
    # if(i>=3){
    #   k3_headFMD_OG <- k_old_headFMD
    #   # imprtsFMDK <- 0
    #   #### Here I will get how many animals are imported from the condition 
