...
```{r}
cost_CowCalf_Tag <- weighted.mean(x = c(4.21, 2.90, 2.73, 2.43, 2.42, 2.43, 2.40), w = c(49,99,499,999,1999,4999,5000))
```
...
```{r}
cost_CowCalf_NTag <- weighted.mean(x = c(5.54, 5.29, 5.20, 5.04, 5.04, 5.04, 5.04), w = c(49,99,499,999,1999,4999,5000))
```
...
Aggregating all the costs. 
<!-- I am using the upperbound from the cow/calf costs i.e., the costs from the operations that currently not tag -->

```{r}
total_Costs <- sum(cost_CowCalf_Tag,cost_CowCalf_NTag, cost_Bckg, cost_Feedlot, cost_Packers)
...
* The quantities remain same when we don't adjust for change in price! (because the prices change by very small magnitude).
